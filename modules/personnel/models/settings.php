    ...
    public function submit(Request $request)
    {
        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if ($login['active'] == 1 && Login::checkPermission($login, 'can_config')) {
                ...
                $config->personnel_status = $request->post('personnel_status', array())->toInt();
                if (empty($config->personnel_status)) {
                    // ไม่ได้เลือกสถานะสมาชิก
                    $ret['ret_personnel_status'] = 'Please select at least one item';
                } else {
                    // save config
